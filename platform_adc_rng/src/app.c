...
// The LSB[2:0] of each ADC sample will be random numbers
// Hence the mask is defined as 0x7 to only select the relevant bits from an
// ADC sample
#define ADC_RND_BIT_MASK            0x7

// The LSB[2:0] of each ADC sample are random numbers. This variable is used
...
/**************************************************************************//**
 * @brief  Initialize the ADC function
 *****************************************************************************/
void initADC(void)
{
  // Enable ADC0 clock
  ...
  // Initialize to max ADC clock for Series 1
...
void app_init(void)
{
  ...
  for (i = 0; i < rounds; i++) {
    ...
    // Get the ADC result and since only LSB[2:0] of a sample will be random
    // numbers, the ADC_RND_BIT_MASK is used to only use the relevant bits and
    // discard the rest
