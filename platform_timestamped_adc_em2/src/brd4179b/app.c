 ...
 */
#define IADC_INPUT          iadcPosInputPortDPin2 // EXP header pin 11
#define ALLOCATE_ABUS       (GPIO->CDBUSALLOC |= GPIO_CDBUSALLOC_CDEVEN0_ADC0)
...
/*
 * Hand-off sequence to allow EFP to power decouple of EFR32xG21.  This
 * function has to be included manually when using GSDK 2.7.6.  Later
 * SDK releases will eventually include this in the EFP driver.
 */
...
void triggerInit(void)
{
  ...
  PRS_SourceAsyncSignalSet(PRS_RTCC_CH,
  ...
  PRS_SourceAsyncSignalSet(PRS_IADC_CH,
                           PRS_ASYNC_CH_CTRL_SOURCESEL_GPIO,
                           TRIG_PIN);
  PRS_ConnectConsumer(PRS_IADC_CH, prsTypeAsync,
...
void app_init(void)
{
  ...
  sl_efp_efp0_init.config_data = config_data[0];
  sl_efp_efp0_init.config_size = (unsigned int) SL_EFP_GEN_SIZE;
  sl_efp_init(sl_efp_efp0, &sl_efp_efp0_init);

  sl_efp_decouple_handoff(sl_efp_efp0, 0, 7, 10);  // BK_IRI_CON=0, BK_TON_MAX=7, BK_IPK=10
