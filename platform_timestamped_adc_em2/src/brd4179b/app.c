...
bool haltOnButton0(void)
{
  ...
  GPIO_PinModeSet(gpioPortB, 0, gpioModeInputPullFilter, 1);

  // Is button 0 pressed? If so, turn on LED0 and return true
  if (GPIO_PinInGet(gpioPortB, 0) == 0) {
...
void iadcInit(void)
{
   ...
   *
   * Conversion Time = ((4 * OSR) + 2) / fCLK_ADC = ((4 * 2) + 2) / 1 MHz = 10 �s
   */
