...
/***************************************************************************//**
 * Weak function override; force power manager to wake/process ISR flags
 ******************************************************************************/
sl_power_manager_on_isr_exit_t app_sleep_on_isr_exit(void)
{
  if ((update_mode == 1) || (update_counter == 1)) {
...
void initGPIO(void)
{
  ...
  GPIO->LESENSEROUTE_SET.CH0OUTROUTE =
    (LCSENSE_PORT << _GPIO_LESENSE_CH0OUTROUTE_PORT_SHIFT)
    | (LCSENSE_PIN << _GPIO_LESENSE_CH0OUTROUTE_PIN_SHIFT);

  // LESENSE pin PB3 set to push pull alternate for excitation
  GPIO_PinModeSet(LCSENSE_PORT, LCSENSE_PIN, gpioModePushPullAlternate, 0);
...
void initLesense(void)
{
  ...
  initStAll.St[0].compVal = 0x01;   // compare match on channel 0 sensor state
  ...
  LESENSE->CH_SET[0].INTERACT =
    (LCSENSE_PIN << _LESENSE_CH_INTERACT_OFFSET_SHIFT);
...
void initPCNT(void)
{
  ...
  initPcnt.countDown = false;            // direction controlled by S1
