# Platform - Interrupt-Driven HFRCO Calibration #
...
![Flash badge](https://img.shields.io/badge/Flash-18.02%20KB-blue)
![RAM badge](https://img.shields.io/badge/RAM-4.63%20KB-blue)
...
You can validate calibration by:

1. Setting a breakpoint in `start_cal()` where the current tuning value is read.
2. Forcing the `HFRCODPLL->CAL.TUNING` field to 0x00 or 0x7F to maximize adjustment iterations.
3. Letting the code run until the breakpoint in `CMU_IRQHandler()` (after CALRDY) and single-stepping the comparison logic.
4. Verifying the up count converges toward the ideal LFXO count (1808 at 19 MHz) and stops when no closer value is possible.

If desired, measure PB1 output with an oscilloscope to observe frequency changes as tuning progresses.
