...
void app_init(void)
{
  ...
  /* enable gpiointerrupt on push button 0 for energy mode switching */
  sl_gpio_t gpio_button0_pin = {
    .port = BUTTON0PORT,
    .pin = BUTTON0PIN
  };

  sl_hal_gpio_set_pin_mode(&gpio_button0_pin, gpioModeInputPullFilter, 1);
  ...
  sl_hal_gpio_set_pin_mode(&gpio_button1_pin, gpioModeInputPullFilter, 1);
  ...
  sl_efp_decouple_handoff(sl_efp_efp0, 0, 7, 10);
#endif
  ...
  while (1) {
    ...
    if (voa_sw_on) {
      /* turn VOA_SW ON */
      sl_efp_set_voa_sw_mode(sl_efp_efp0, efp_voa_sw_mode_on);
    } else {
      /* turn VOA_SW OFF */
      sl_efp_set_voa_sw_mode(sl_efp_efp0, efp_voa_sw_mode_off);
    }
#endif
    if (enterEM2) {
