...
static void initGPIO(void)
{
  ...
  GPIO_ExtIntConfig(PB0_PORT, PB0_PIN, PB0_PIN, true, false, false);
...
void app_process_action(void)
{
  ...
  if (app_status.buffer_ready) {
    ...
    const volatile uint32_t *processBuffer;

    // Process the buffer not being written to by the LDMA
    if (app_status.buffer_select) {
      processBuffer = Micbuffer1;
    } else {
      processBuffer = Micbuffer2;
    }
    ...
    for (uint16_t counter = 0; counter < FFT_SIZE; counter++) {
      ...
      timeDomBuffer[counter] =
        (float) (((int32_t) __REV(processBuffer[counter])) >> 12);
    }
