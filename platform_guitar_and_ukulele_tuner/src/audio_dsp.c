 ...
 ********************************/
static uint32_t Check_2nd_harmonic(float *binBuffer,
                                   uint32_t bin_index,
                                   float maxval);
...
float DSP_AnalyzeData(float *dataBuffer, float *binMagBuffer, float fft_res)
{
  ...
  arm_max_f32(&binMagBuffer[BIN_OFFSET],
              (FFT_SIZE / 2) - BIN_OFFSET,
              &maxval,
              &bin_index);
...
{
  for (uint16_t count = 0; count < FFT_SIZE; count++) {
    Hann_coeff[count] = 0.5
                        - (0.5
                           * arm_cos_f32(2 * PI
                                         * ((float)count
                                            / ((float)FFT_SIZE - 1))));
                                   ...
                                   float maxval)
{
  ...
  return ((half_frequency_maxval * 100) / maxval > 10
          ? half_frequency_index : original_bin_index);
}
