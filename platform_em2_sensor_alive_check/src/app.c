...
//  LETIMER0->ROUTEPEN |= LETIMER_ROUTEPEN_OUT1PEN;
  ...
  LETIMER_CompareSet(LETIMER0, 1,
                     CMU_ClockFreqGet(cmuClock_LETIMER0) * DUTY_CYCLE
                     / (OUT_FREQ * 100));
}
...
void initPrs(void)
{
  ...
  PRS_SourceSignalSet(PRS_BASE_CH,
                      PRS_CH_CTRL_SOURCESEL_PRSL,
                      PRS_CH_CTRL_SIGSEL_PRSCH0,
                      prsEdgePos);

  PRS_SourceAsyncSignalSet(PRS_BASE_CH,
                           PRS_CH_CTRL_SOURCESEL_LETIMER0,
                           PRS_CH_CTRL_SIGSEL_LETIMER0CH0);                                               // POLL
  PRS_SourceAsyncSignalSet(PRS_BASE_CH + 1,
                           PRS_CH_CTRL_SOURCESEL_GPIOH,
                           BSP_GPIO_STATUS_PIN - 8);                                                  // STATUS
  // Configure PRS Logic
...
void initGPIO(void)
{
  ...
  GPIO_PinModeSet(PORTIO_LETIMER0_OUT0_PORT,
                  PORTIO_LETIMER0_OUT0_PIN,
                  gpioModePushPull,
                  0);
  GPIO_PinModeSet(BSP_GPIO_POLL_STATUS_PORT,
                  BSP_GPIO_POLL_STATUS_PIN,
                  gpioModePushPull,
                  0);

  GPIO_PinModeSet(BSP_GPIO_STATUS_PORT, BSP_GPIO_STATUS_PIN, gpioModeInput, 1);// gpioModeInputPullFilter
  GPIO_ExtIntConfig(BSP_GPIO_STATUS_PORT,
                    BSP_GPIO_STATUS_PIN,
                    BSP_GPIO_STATUS_PIN,
                    0,
                    0,
