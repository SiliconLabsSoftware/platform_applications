...
sl_beacon_power_level_t decide_power_level(sl_harvester_voltages_t *hv)
{
  ...
  if (hv->storage_voltage_millivolts > VCHRDY) {
    ...
    if (hv->delta_storage_voltage_millivolts >= MIN_CHARGING_DELTA_V) {
      ...
      return (hv->source_voltage_millivolts
              >= VSOURCE_LIMIT) ? POWER_LEVEL_MAX : POWER_LEVEL_HIGH;
    } else {
  ...
  if (hv->delta_storage_voltage_millivolts >= MIN_CHARGING_DELTA_V) {
    return (hv->source_voltage_millivolts
            >= VSOURCE_LIMIT) ? POWER_LEVEL_MEDIUM : POWER_LEVEL_MIN;
  }
  ...
  sl_beacon_settings_t *settings)
{
  ...
  if (BURAM->RET[BURAM_KEY_IDX].REG == BURAM_KEY) {
    ...
    hv->delta_storage_voltage_millivolts = hv->storage_voltage_millivolts
                                           - BURAM->RET[BURAM_LAST_STO_IDX].REG;
