...
int main(void)
{
  ...
  sl_beacon_power_level_t power_level = decide_power_settings_and_update_buram(
    &harvester_voltages,
    &beacon_settings);
  btConfig.txPower = apply_power_settings(mode_select,
                                          power_level,
                                          &beacon_settings);
  ...
  if (beacon_settings.skip_next == false) {
    ...
    if (beacon_settings.short_payload == false) {
      ...
      harvester_payload.deltaVcap_mV =
        harvester_voltages.delta_storage_voltage_millivolts;
      harvester_payload.lightIntensity =
        harvester_voltages.source_voltage_millivolts * 255 / 4000;
